...
class BudgetEditorViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, CoachMarksControllerDataSource {
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "showTaskEditorView" {
            ...
            let taskEditorVC: TaskEditorViewController = (navController?.topViewController as? TaskEditorViewController)!
    ...
    func updateTimeRemaining() {
        ...
        for category in (currentBudget?.categories)! {
            for task in category.tasks {
                taskTimeList.append(Time(newTime: task.timeBudgeted))
            }
        }

        // Subtract all of the time that has been allocated.
        for taskTime in taskTimeList {
            newTime.hours -= taskTime.hours
            newTime.minutes -= taskTime.minutes
        }
